...
/**
 * Mock implementation of the SimpleTest class for testing purposes. This class provides a mock
...
public class SimpleTest extends AbstractForm {
  ...
  /** The my action. */
  @Autowired private BackupAction myAction;

  private static final String documentName = "VALID.pdf";

  private static final String versionName = "firstVersion.pdf";
  private static final String versionContent = "new content";
  private static final String versionLabel = "1.0";

  private static final String version2Name = "secondVersion.pdf";
  private static final String version2Content = "new content 2";
  private static final String version2Label = "1.1";

  private static final String version3Name = "thirdVersion.pdf";
  private static final String version3Content = "new content 3";
  private static final String version3Label = "2.0";
  ...
  public void init() {
    ...
    String nodeName =
        (String) serviceRegistry.getNodeService().getProperty(docs.getNodeRefs().get(0), PROP_NAME);
  ...
  public void versioned() {
    ...
    Map<QName, Serializable> propertiesFrozen =
        nodeService.getProperties(version.getFrozenStateNodeRef());
    Assert.assertEquals("prop name", version3Name, propertiesFrozen.get(PROP_NAME));
    Assert.assertEquals(
        "prop version label", version3Label, propertiesFrozen.get(PROP_VERSION_LABEL));
    ...
    Map<QName, Serializable> properties2 =
        nodeService.getProperties(version2.getVersionedNodeRef());
    Map<QName, Serializable> propertiesFrozen2 =
        nodeService.getProperties(version2.getFrozenStateNodeRef());
    ...
    Assert.assertEquals(
        "prop version label", version2Label, propertiesFrozen2.get(PROP_VERSION_LABEL));
    Assert.assertEquals(
        "prop version type", MINOR.name(), propertiesFrozen2.get(PROP_VERSION_TYPE));
    ...
    Map<QName, Serializable> properties3 =
        nodeService.getProperties(version3.getVersionedNodeRef());
    Map<QName, Serializable> propertiesFrozen3 =
        nodeService.getProperties(version3.getFrozenStateNodeRef());
    ...
    Assert.assertEquals(
        "prop version label", versionLabel, propertiesFrozen3.get(PROP_VERSION_LABEL));
    Assert.assertEquals(
        "prop version type", MINOR.name(), propertiesFrozen3.get(PROP_VERSION_TYPE));
