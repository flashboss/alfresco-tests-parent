...
process_branch() {
    ...
    if ! mvn clean compile -DskipTests 2>&1 | tee /tmp/mvn_compile.log; then
        # Controlla se gli errori sono in MockRuleService.java (errori comuni che possiamo correggere)
        if grep -q "MockRuleService.java" /tmp/mvn_compile.log && grep -q "saveRule\|does not override" /tmp/mvn_compile.log; then
            echo "Detected common MockRuleService errors, attempting to fix..."
            if [ -f "fix_mock_rule_service.sh" ]; then
                ./fix_mock_rule_service.sh
                echo "Recompiling after fixes..."
                if ! mvn clean compile -DskipTests; then
                    echo "=========================================="
                    echo "ERROR: Compilation still failed after automatic fixes for branch $branch"
                    echo "Please fix compilation errors manually."
                    echo "=========================================="
                    git checkout -- pom.xml */pom.xml 2>/dev/null || true
                    return 1
                fi
                echo "Compilation successful after automatic fixes!"
            else
                echo "=========================================="
