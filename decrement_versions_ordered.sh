...
process_branch() {
    ...
    # Compila di nuovo con la nuova versione per verificare che funzioni
    ...
    if ! mvn clean compile -DskipTests; then
        ...
        echo "Please fix the compilation errors manually, then:"
        echo "1. Fix the errors in the code"
        echo "2. Run: mvn clean compile -DskipTests"
        echo "3. Run: git add -A && git commit --amend --no-edit"
        echo "4. Run: git push origin $branch --force"
        echo "5. Run: git tag $new_version && git push origin $new_version"
        echo "=========================================="
        echo "Current branch: $branch"
        echo "New version: $new_version"
        echo "Changes have been committed but compilation failed."
        echo "You are now on branch $branch - fix errors and continue manually."
        return 1
    fi
