...
/**
 * Mock implementation of the WorkflowOKTest class for testing purposes. This class provides a mock
...
public class WorkflowOKTest extends ComplexAbstractForm {
  ...
  @Deployment(
      resources = {
        "alfresco/module/alfresco-tests-activiti-sample/workflow/ComplexRaRProcess.bpmn"
      })
  /**
   * Performs test human.
   *
  ...
  public void testHuman() throws ParseException {
    ...
    List<Task> irarSubscription =
        taskService
            .createTaskQuery()
            .taskDefinitionKey("irarSubscription")
            .includeProcessVariables()
            .list();
    ...
    // I verify that the two files have been created in the new rar folder
    String generationFolderName =
        (String) nodeService.getProperty(generationFolder, ContentModel.PROP_NAME);
    ResultSet resultQ =
        searchService.query(
    ...
    Assert.assertTrue(
        "Added zip file to new RAR folder",
        nodeService
            .getPath(createdNodeRef)
            .toString()
            .endsWith(
    ...
    Assert.assertTrue(
        "Added xml file in new RAR folder",
        nodeService
            .getPath(createdNodeRef)
            .toString()
            .endsWith(
        ...
        nodeService
            ...
            .endsWith(
                "workspace/SpacesStore/company_home/sites/digital-conservation-complex-bank/documentLibrary/sas/"
                    + generationFolderName));
    Assert.assertTrue(
        "Empty folder inside store", nodeService.getChildAssocs(createdNodeRef).isEmpty());
    ...
    resultQ =
        searchService.query(
            StoreRef.STORE_REF_WORKSPACE_SPACESSTORE,
            SearchService.LANGUAGE_FTS_ALFRESCO,
            "PATH:\"pkg_919f220e-870a-4c56-ba11-5030ee5325f0/sas_complex_"
                + generationFolderName
                + ".zip\"");
    createdNodeRef = resultQ.getNodeRef(0);
    Assert.assertTrue(
        "Zip file inside activiti folder",
        nodeService
            .getPath(createdNodeRef)
            .toString()
            .endsWith(
                "workspace/SpacesStore/workflow/packages/pkg_919f220e-870a-4c56-ba11-5030ee5325f0/sas_complex_"
                    + generationFolderName
                    + ".zip"));
    ...
    createdNodeRef = resultQ.getNodeRef(0);
    Assert.assertTrue(
        "XML signed in the new rar folder",
        nodeService
            .getPath(createdNodeRef)
            .toString()
            .endsWith(
