...
/**
 * Mock implementation of the SimpleActivitiTest class for testing purposes. This class provides a
...
public class SimpleActivitiTest extends AbstractActivitiForm {
  ...
  public static final String ACTIVITY_KEY = "generationWorkflow";

  /** Default admin user to start the scheduler process */
  private static final String ADMIN_USER_NAME = "kermit";

  /** Default traveler user to work with the reservations */
  private static final String USER_NAME = "gonzo";
  ...
  public void init(Map<String, Object> variables) {
    ...
    NamespaceService namespaceService =
        activitiProcessEngineConfiguration.getServiceRegistry().getNamespaceService();
    ...
    NodeService nodeService =
        activitiProcessEngineConfiguration.getServiceRegistry().getNodeService();
    ...
    try {
      ...
      properties.put(
          ...
          QName.createQName("mycont", "contractsPdvCons", namespaceService));
      insertZip(
          generationFolder,
          "contracts_" + generationFolderName + ".zip",
          "document",
          "text",
          properties);
    } catch (IOException e) {
  ...
  public void initDemoUsers(IdentityService identityService) {
    createUser(
        ...
        "The Frog",
        ADMIN_USER_NAME,
        ADMIN_USER_NAME + "@activiti.org",
        null,
        asList(CONTRIBUTORS, "user", "admin"),
        asList(
            "birthDate",
            "10-10-1955",
            "jobTitle",
            ...
            "alfresco",
            "skype",
            "activiti_" + ADMIN_USER_NAME + "_frog"));
    createUser(
        identityService,
        USER_NAME,
        "Gonzo",
        "The Great",
        USER_NAME,
        USER_NAME + "@activiti.org",
        null,
        asList(CONTRIBUTORS, "user"),
  ...
  @Deployment(
      resources = {"alfresco/module/alfresco-tests-activiti-sample/workflow/SimpleProcess.bpmn"})
  /**
  ...
  public void testWorkflow() throws ParseException {
    ...
    MockActivitiScriptNode activitiScriptNode =
        new MockActivitiScriptNode(generationFolder, serviceRegistry);
    ...
    List<Task> selectedSaS =
        taskService
            .createTaskQuery()
            .taskDefinitionKey("selectedSaS")
    ...
    ResultSet resultQ =
        searchService.query(
            ...
            "PATH:\"rar/contracts_" + generationFolderName + ".zip\"");
    NodeRef createdNodeRef = resultQ.getNodeRef(0);
    String path = nodeService.getPath(createdNodeRef).toString();
    Assert.assertTrue(
        "Added a zip file in the RAR folder",
        path.endsWith(
            "workspace/SpacesStore/company_home/sites/simple-site/documentLibrary/rar/contracts_"
    ...
    resultQ =
        searchService.query(
            StoreRef.STORE_REF_WORKSPACE_SPACESSTORE,
            SearchService.LANGUAGE_FTS_ALFRESCO,
            "PATH:\"pkg_919f220e-870a-4c56-ba11-5030ee5325f0/contracts_"
                + generationFolderName
                + ".zip\"");
    createdNodeRef = resultQ.getNodeRef(0);
    path = nodeService.getPath(createdNodeRef).toString();
    Assert.assertTrue(
        "File zip in the activiti folder",
        path.endsWith(
            "workspace/SpacesStore/workflow/packages/pkg_919f220e-870a-4c56-ba11-5030ee5325f0/contracts_"
                + generationFolderName
                + ".zip"));
