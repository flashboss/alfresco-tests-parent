...
public class PreviousWSSample extends DeclarativeWebScript {
				...
				for (FormData.FormField field : fields) {
					switch (field.getName()) {
						...
						case "codicews":
							...
							folderWSSamples.add(
									nodeService.getChildByName(rootNode, ContentModel.ASSOC_CONTAINS, codiceWSSample));
							break;
						case "automatico":
							/** The codice w s sample. */
	...
	private List<NodeRef> getPosizionidaConservareAsNodeRef(DateTime dateFrom, DateTime dateTo) {
		ResultSet folderRs = serviceRegistry.getSearchService().query(this.storeRef,
				...
				repositoryFolderTemplateWSSample.replace("{wsSampleFrom}", fmt.format(dateFrom.toDate())).replace(
						"{wsSampleTo}",
	 ...
	 *
	 * @param repositoryFolderTemplateWSSample the repository folder template w s
	 *                                         sample
	 */
	 ...
	 *
	 * @param documentsWSSampleFolderTemplate the documents w s sample folder
	 *                                        template
	 */
