...
/**
 * Mock implementation of the NodeListDownloadWebScript class for testing purposes. This class
...
// implements InitializingBean
{
   ...
   * @return the result
   */
  protected List<NodeRef> identifyResource(
      String format, WebScriptRequest req, int maxItems, int skypCount) {
  ...
  protected List<Pair<QName, Boolean>> buildPropertiesForHeader(
    ...
    for (String qnameString : modelProperties) {
      ...
      try {
        properties.add(
            new Pair<QName, Boolean>(QName.createQName(prefix, localName, namespaceService), true));
            ...
            if (assocs.size() > 0) {
              ...
              for (AssociationRef ref : assocs) {
                ...
                if (ContentModel.TYPE_PERSON.equals(type)) {
                  ...
                  NodeRef linkRef =
                      (NodeRef) nodeService.getProperty(child, ContentModel.PROP_LINK_DESTINATION);
                  if (linkRef != null) {
                    if (text.length() > 0) {
                      text.append('\n');
                      lines++;
  ...
  public void generateSpreadsheet(
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "Executing NodeList WS with parameters: skipCount= "
              + skipCount
              + " , maxItems= "
              + maxItems
              + " , maxLines= "
    ...
    if ("csv".equals(format)) {
      ...
      do {
        ...
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Executing iteration of resources with num: " + num + " and skipCount: " + skipCount);
          if (iteration == maxIterations) logger.debug("Exiting because max iteration is reached");
        }
  ...
  public static class WriteExcel {
     ...
     */
    private WriteExcel(
        WebScriptResponse res, Map<String, Object> model, String format, String filenameBase) {
      this.res = res;
      this.model = model;
      this.format = format;
