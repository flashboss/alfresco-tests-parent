...
/**
 * Mock implementation of the MockSearchService class for testing purposes. This class provides a
...
public class MockSearchService implements SearchService, Serializable {
  ...
  public ResultSet query(StoreRef store, String language, String query) {
    ...
    if (language.equals(SearchService.LANGUAGE_XPATH)) XPATHQuery(store, query, nodeRefs, rows);
    else if (language.equals(SearchService.LANGUAGE_FTS_ALFRESCO)
        || language.equals(SearchService.LANGUAGE_LUCENE)) FTSQuery(store, query, nodeRefs, rows);
  ...
  public ResultSet query(SearchParameters searchParameters) {
    ...
    if (maxItems > -1 && resultSet.length() > maxItems) {
      List<ResultSetRow> rows = new ArrayList<ResultSetRow>();
      for (int i = 0; i < maxItems; i++) rows.add(new MockResultSetRow(resultSet.getNodeRef(i)));
  ...
  public List<NodeRef> selectNodes(
    ...
    ResultSet resultSet =
        query(contextNodeRef.getStoreRef(), SearchService.LANGUAGE_XPATH, xpath, parameters);
    for (ResultSetRow row : resultSet) result.add(row.getNodeRef());
    return result;
  }
  ...
  public boolean contains(
      NodeRef nodeRef, QName propertyQName, String googleLikePattern, Operator defaultOperator)
      throws InvalidNodeRefException {
    // TODO Auto-generated method stub
    return false;
  }
  ...
  public class MockResultSet implements ResultSet {
    ...
    private Map<NodeRef, List<Pair<String, List<String>>>> highLights =
        new HashMap<NodeRef, List<Pair<String, List<String>>>>();
    ...
    public List<NodeRef> getNodeRefs() {
      List<NodeRef> result = new ArrayList<NodeRef>();
      for (ResultSetRow row : rows) result.add(row.getNodeRef());
      return result;
    }
  ...
  private List<MockProperty> getPropertiesFromQuery(String query) {
    ...
    for (String segm : segments) {
      ...
      if (!seg.startsWith("PATH:")
          && !seg.substring(1, seg.length()).startsWith("PATH:")
          && !seg.startsWith("TYPE:")
          && !seg.contains("[")
        ...
        properties.add(
            new MockProperty(
                key,
                splitted[2].replaceAll("\"", "").replaceAll("\\\\", ""),
                seg.startsWith("-")
                        || seg.startsWith("(-")
                        || seg.toLowerCase().startsWith("not")
                        || seg.toLowerCase().startsWith("(not")
                    ? true
                    : false));
      }
    ...
    for (MockProperty property : properties) {
      ...
      if (value instanceof String || value == null) {
        ...
        if (!result) {
          ...
          if (!property.isToDelete()) results.add(result);
          else results.add(!result);
        }
      } else if (!property.isToDelete()) results.add(true);
      else results.add(false);
    }
    boolean result = true;
    for (boolean rs : results) result = result && rs;
    return result;
  }
  ...
  private boolean hasAspects(List<MockAspect> aspects, NodeRef nodeRef) {
    ...
    for (MockAspect aspect : aspects) {
      ...
      if (aspect.isToDelete()) results.add(!result);
      else results.add(result);
    }
    boolean result = true;
    for (boolean rs : results) result = result && rs;
  ...
  private boolean hasPath(
      StoreRef store, String path, String[] subpaths, int wildcardsNumber, NodeRef nodeRef) {
    String nodepath = getNodeService().getPathAsString(nodeRef);
    if (store == null || path == null) return true;
    else if (store != null
        && !nodepath.contains(MockContentService.FOLDER_TEST + store.getProtocol())) return false;
    nodepath =
        nodepath.substring(
            nodepath.indexOf(MockContentService.FOLDER_TEST)
                + MockContentService.FOLDER_TEST.length());
    String lastNodepath = "";
    ...
    for (int i = 0; i < subpaths.length; i++) {
      ...
      if (result
        ...
        if (!subpath.endsWith("/")) {
          ...
          if (lastNodepath.equals(nameSubpath)) result = false;
        }
    }
    return result;
  }
   ...
   * @param rows the rows
   */
  private void XPATHQuery(
      StoreRef store, String query, List<NodeRef> nodeRefs, List<ResultSetRow> rows) {
  ...
  private void FTSQuery(
    ...
    for (NodeRef nodeRef : nodeRefs) {
      if (hasType(type, nodeRef)
          && hasProperties(properties, nodeRef)
          && hasAspects(aspects, nodeRef)
  ...
  private String prepare(String query, StoreRef store) {
    ...
    if (store != null) prefix = prefix + store.getProtocol();
    int index = query.indexOf(prefix);
    if (index >= 0) query = query.substring(index + prefix.length());
    String result = "";
    String[] slashes = query.split("/");
    for (int i = 0; i < slashes.length; i++) {
      String slashed = slashes[i];
      if (slashed.contains(":")) result += slashed.split(":")[1];
      else result += slashed;
      result += "/";
    }
