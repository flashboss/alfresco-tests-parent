...
/**
 * A HashMap extension that filters out certain node references based on their file path. Used to
 * exclude root store paths from the node reference map.
 *
 * @author vige
 */
public class FilteredHashMap extends HashMap<NodeRef, File> {
  ...
  public void putAll(Map<? extends NodeRef, ? extends File> m) {
    for (NodeRef key : m.keySet()) {
      if (haveToAdd(m.get(key).getAbsolutePath())) super.put(key, m.get(key));
    }
  }
  ...
  private void removeFilteredValues() {
    for (NodeRef key : keySet()) {
      if (!haveToAdd(get(key).getAbsolutePath())) remove(key);
    }
  }
